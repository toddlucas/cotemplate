...
import { createUserMessageEvent, createAIResponseEvent } from '../events';
...
describe('Platform Container', () => {
  ...
  describe('Event System Initialization', () => {
    it('should initialize successfully without automatic EventStore-EventBus connection', async () => {
      ...
      const aiEvent = createUserMessageEvent('conv-1', 'Hello, AI!');
      ...
      const storedEvents = await eventStore.queryEvents((event) =>
        'conversationId' in event && event.conversationId === 'conv-1'
      );
      expect(storedEvents).toHaveLength(1);
      expect(storedEvents[0]).toEqual(aiEvent);
    });
    ...
    it('should allow manual event publishing when needed', async () => {
      ...
      const event1 = createUserMessageEvent('conv-1', 'Message 1');
      const event2 = createAIResponseEvent('conv-1', 'Response 1', 'gpt-4', 'resp-1', true);
