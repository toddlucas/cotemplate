...
export const selectIsFirstPage = (state: ItemState) => {
  return state.tableState.currentPage === 0;
};

export const selectIsLastPage = (state: ItemState) => {
  return state.tableState.currentPage >= Math.ceil(state.totalCount / state.tableState.pageSize) - 1;
};
...
export const selectPaginationInfo = (state: ItemState) => {
  ...
  const startItem = state.tableState.currentPage * state.tableState.pageSize + 1;
  const endItem = Math.min((state.tableState.currentPage + 1) * state.tableState.pageSize, state.totalCount);
  ...
  return {
    ...
    startItem,
    endItem,
    hasNextPage: state.tableState.currentPage < totalPages - 1,
    hasPrevPage: state.tableState.currentPage > 0,
  };
};
